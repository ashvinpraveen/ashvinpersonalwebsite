...
import { BookOpen, Bot, ChevronLeft, Inbox, MailOpen, Send, UserRound } from "lucide-react";
...
function DiaryAdminInbox({ adminSecret }: { adminSecret: string }) {
  const conversations = useQuery(api.diary.listForAdmin, { adminSecret });
  const [selectedThreadId, setSelectedThreadId] = useState<Id<"diaryThreads"> | null>(null);
  ...
  const threadDetail = useQuery(
    api.diary.getThreadForAdmin,
    adminSecret && selectedThreadId ? { adminSecret, threadId: selectedThreadId } : "skip",
  );
  const markThreadRead = useMutation(api.diary.markThreadReadForAdmin);
  ...
  if (conversations === undefined) {
    return (
      <div className="flex h-full items-center justify-center">
        <p className="font-mono text-xs text-muted-foreground">Loading diary chats...</p>
      </div>
    );
  }
  ...
  if (visibleConversations.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center bg-background/50 px-5 text-center">
        ...
        <p className="text-sm font-medium text-foreground">No diary chats yet.</p>
        <p className="mt-1 max-w-sm text-sm text-muted-foreground">
          New diary conversations will show up here after visitors write on the page.
        </p>
      </div>
    );
  }
      ...
      >
        <div className="flex h-14 shrink-0 items-center justify-between border-b border-border px-4">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium">Diary chats</p>
          </div>
            ...
            return (
              ...
              >
                <div className="min-w-0 space-y-1">
                  ...
                  >
                    {conversation.latestMessage?.body ?? "New diary chat"}
                  </p>
                </div>
                <time
                  className={cn(
      ...
      >
        {selectedThread && threadDetail ? (
            ...
            <header className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-border bg-card px-4">
              <div className="flex min-w-0 items-center gap-2">
                <Button
                  ...
                  aria-label="Back to diary chats"
          ...
          <div className="flex flex-1 flex-col items-center justify-center px-5 text-center">
            ...
            <p className="text-sm text-muted-foreground">Select a diary chat.</p>
          </div>
        )}
      </section>
    </div>
...
export default function DiaryAdmin() {
  if (!isAdminEnabled) {
    return (
      <main className="flex h-dvh items-center justify-center bg-background p-6">
        <FeatureUnavailable
          title="Diary admin is not configured"
          description="Set NEXT_PUBLIC_CONVEX_URL and run Convex to enable the private diary inbox."
        />
      </main>
    );
  }
  ...
  return (
    <AdminShell
      activePath="/admin/diary"
      title="Diary inbox"
    >
      {(adminSecret) => <DiaryAdminInbox adminSecret={adminSecret} />}
    </AdminShell>
  );
}
